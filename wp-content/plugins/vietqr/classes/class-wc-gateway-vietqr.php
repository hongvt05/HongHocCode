...
public function thankyou_page($order_id)
{
    ...
    // Kiểm tra nếu đơn hàng đã hoàn thành (Completed)
    if ($order && $order->has_status('completed')) {
        echo '<div style="text-align: center; padding: 20px; border: 2px solid #28a745; border-radius: 10px; margin-bottom: 20px; background-color: #f9fff9;">';
        echo '  <h2 style="color: #28a745;">🎉 Thanh toán thành công!</h2>';
        echo '  <p>Cảm ơn bạn. Chúng tôi đã nhận được thanh toán và đơn hàng của bạn đang được xử lý.</p>';
        echo '</div>';
        return; 
    }

    // Nếu chưa hoàn thành thì hiện như mặc định của plugin
